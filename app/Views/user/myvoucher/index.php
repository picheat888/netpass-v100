...
<!-- DataTables server-side: ตาราง voucher ของผู้ใช้ -->
<script>
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dt = NetPass.dataTable('#voucherTable', {
        ...
        action: '#vfAction',
        ajax: {
            url: '<?= site_url('myvoucher/data') ?>',
            data: function (d) { d.loc = locSel.value; d.status = statSel.value; }
        },
        order: [[4, 'desc']],   // เรียงตามวันที่ขอ ล่าสุดก่อน
        columns: [
    ...
    selPrintBtn.addEventListener('click', async function () {
        ...
        try {
            const url = '<?= site_url('myvoucher/tickets') ?>?' + [...selected].map((id) => 'ids[]=' + encodeURIComponent(id)).join('&');
