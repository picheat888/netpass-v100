...
/**
 * VoucherController (Admin) — ประวัติการออก voucher ทั้งหมด
 * ตารางเป็น DataTables server-side: index() ส่งแค่หน้า, data() ส่ง JSON ให้ DataTables
 */
class VoucherController extends BaseController
{
    // palette สีประจำพื้นที่ (by index) — ใช้กับจุดสี/avatar ให้ตรงกับ mockup
    ...
    public function index()
    {
        ...
        return view('admin/voucher/index', [
            'title'     => lang('Voucher.histTitle'),
            'subtitle'  => lang('Voucher.histSub'),
            'active'    => 'voucher',
            'locations' => $locModel->findAll(),
        ]);
    }

    // ข้อมูลตารางสำหรับ DataTables (server-side processing) → คืน JSON
    public function data()
    {
        ...
        // map ดัชนีคอลัมน์ของ DataTables → คอลัมน์ SQL ที่ sort ได้
        // คอลัมน์ 0 เป็น checkbox (เลือกเพื่อพิมพ์) → คอลัมน์ข้อมูลเลื่อน index +1
        $orderCols = [
        ...
        $recordsTotal = $issueModel->countAllResults();

        // builder หลัก + join + ตัวกรอง
        $builder = $issueModel
    ...
    // สร้าง HTML ของแต่ละแถว (8 คอลัมน์) — ใช้ markup เดียวกับดีไซน์เดิมเป๊ะ
    private function renderRow(array $row, bool $isEn): array
    {
        ...
        $name        = $reqFull !== '' ? $reqFull : $username;
        $initialBase = trim(($row['req_firstname'] ?? '') . ($row['req_lastname'] ?? '')) ?: $username;
        $initial     = mb_strtoupper(mb_substr(trim($initialBase) !== '' ? $initialBase : 'G', 0, 1));
        $ln          = $isEn ? (($row['loc_name_en'] ?? '') ?: $row['loc_name']) : $row['loc_name'];

        // สีพื้นที่ — by index ของ location_id ในชุด palette (อิงลำดับ id ที่โหลดมา)
        $locModel = new LocationModel();
        ...
        // คอลัมน์ 0: ผู้ขอ (avatar + ชื่อเต็ม + username)
        $avatar = ! empty($row['req_img'])
            ? '<img src="' . esc(base_url($row['req_img']), 'attr') . '" alt="">'
            : esc($initial);
        $c0 = '<div class="d-flex align-items-center gap-2">'
            . '<span class="np-avatar" style="width:34px;height:34px;font-size:12.5px">' . $avatar . '</span>'
            . '<div class="min-w-0">'
            . '<div class="fw-semibold text-truncate" style="max-width:160px">' . esc($name) . '</div>'
            . '<div class="np-stat-sub font-mono">' . esc($username) . '</div>'
            . '</div></div>';
        ...
        // คอลัมน์ 7: ปุ่มดู voucher (เปิด modal) — เก็บ data-attribute ครบเหมือนเดิม
        ...
        return [$cChk, $c0, $c1, $c2, $c3, $c4, $c5, $c6, $c7];
    }

    // คืนข้อมูลตั๋วของ id ที่เลือก (GET ?ids[]=) → JSON สำหรับพิมพ์หลายใบ
    public function tickets()
    {
        ...
        // map สีพื้นที่ ให้ตรงกับจุดสีในตาราง
        $colorMap = [];
